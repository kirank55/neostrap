...
export const SWITCH_USAGE_CODE = `import { NeoSwitch } from "@/components/ui/NeoSwitch

export function SwitchDemo() {
  return <NeoSwitch>Toggle me</NeoSwitch>
}`

export const INSTALL_CLI_CODE = `npx shadcn@latest add https://neostrapui.pages.dev/r/neoswitch.json`
...
const switchVariants = cva(
  "peer inline-flex shrink-0 cursor-pointer items-center rounded-full transition-all disabled:cursor-not-allowed disabled:opacity-50 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]",
  {
    variants: {
      variant: {
        brutal: \`bg-[var(--primary)] \${DOLLAR}{baseBrutalStyle}\`,
        danger: \`bg-red-500 \${DOLLAR}{baseBrutalStyle} data-[state=checked]:bg-red-700\`,
        success: \`bg-green-500 \${DOLLAR}{baseBrutalStyle} data-[state=checked]:bg-green-700\`,
        inverter: \`bg-black hover:bg-[var(--color-bg)] \${DOLLAR}{baseBrutalStyle} data-[state=checked]:bg-[var(--color-bg)]\`,
