				...
				const tween = gsap.to(floatingEl, {
					...
					scrollTrigger: {
						...
						onUpdate: (self) => {
							...
							const headerOpacity =
								progress > 0.85 ? (progress - 0.85) / 0.15 : 0;
							(targetH2 as HTMLElement).style.opacity = String(headerOpacity);
							if (targetHeader) {
								(targetHeader as HTMLElement).style.opacity = String(
									headerOpacity
								);
							}
						},
			...
			default:
				return (
					<div className="text-4xl">
						{COMPONENT_CARDS.find((card) => card.name === componentName)?.icon}
					</div>
				);
		}
	};
